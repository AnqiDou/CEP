...
IF OBJECT_ID ('message_conversations', 'U') IS NULL BEGIN
CREATE TABLE message_conversations (
    ...
    CONSTRAINT uq_message_conversations_item_pair UNIQUE (
        item_id,
        buyer_user_id,
        seller_user_id
    )
...
IF OBJECT_ID ('message_records', 'U') IS NULL BEGIN
...
IF OBJECT_ID ('message_notifications', 'U') IS NULL BEGIN
...
IF COL_LENGTH ('message_records', 'biz_type') IS NULL BEGIN
ALTER TABLE message_records
ADD biz_type NVARCHAR (40) NULL;

END;
GO

IF COL_LENGTH ('message_records', 'biz_id') IS NULL BEGIN
ALTER TABLE message_records
ADD biz_id BIGINT NULL;
...
IF NOT EXISTS (
    ...
    FROM sys.indexes
    WHERE
        name = 'idx_message_conversations_buyer'
        AND object_id = OBJECT_ID ('message_conversations')
) BEGIN
CREATE INDEX idx_message_conversations_buyer ON message_conversations (
    buyer_user_id,
...
IF NOT EXISTS (
    ...
    FROM sys.indexes
    WHERE
        name = 'idx_message_notifications_user'
        AND object_id = OBJECT_ID ('message_notifications')
) BEGIN
CREATE INDEX idx_message_notifications_user ON message_notifications (
    user_id,
    is_read,
    created_at DESC
);
...
IF NOT EXISTS (
    ...
    FROM sys.indexes
    WHERE
        name = 'idx_message_notifications_item'
        AND object_id = OBJECT_ID ('message_notifications')
) BEGIN
CREATE INDEX idx_message_notifications_item ON message_notifications (
    related_item_id,
    created_at DESC
);
...
IF NOT EXISTS (
    ...
    FROM sys.indexes
    WHERE
        name = 'idx_message_conversations_seller'
        AND object_id = OBJECT_ID ('message_conversations')
) BEGIN
CREATE INDEX idx_message_conversations_seller ON message_conversations (
    seller_user_id,
    last_message_at DESC
);
...
IF NOT EXISTS (
    ...
    FROM sys.indexes
    WHERE
        name = 'idx_message_records_conversation'
        AND object_id = OBJECT_ID ('message_records')
) BEGIN
CREATE INDEX idx_message_records_conversation ON message_records (
    conversation_id,
    created_at ASC,
    id ASC
);
