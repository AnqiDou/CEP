IF OBJECT_ID ('dbo.idle_items', 'U') IS NOT NULL BEGIN
DROP TABLE idle_items;

END;
GO

IF OBJECT_ID ('dbo.idle_item_photos', 'U') IS NOT NULL
AND OBJECT_ID ('dbo.item_photos', 'U') IS NULL BEGIN EXEC sp_rename 'dbo.idle_item_photos',
'item_photos';
...
IF OBJECT_ID ('dbo.idle_item_photos', 'U') IS NOT NULL
AND OBJECT_ID ('dbo.item_photos', 'U') IS NOT NULL BEGIN
...
ALTER TABLE items
ADD publisher_user_id BIGINT NULL;
...
UPDATE i
SET
    i.publisher_user_id = d.publisher_user_id
FROM items i
    INNER JOIN item_details d ON d.item_id = i.id
WHERE
    i.publisher_user_id IS NULL
    AND d.publisher_user_id IS NOT NULL;
GO

IF OBJECT_ID ('dbo.item_photos', 'U') IS NULL BEGIN
...
IF NOT EXISTS (
    ...
    WHERE
        ...
        AND object_id = OBJECT_ID ('dbo.item_photos')
) BEGIN
